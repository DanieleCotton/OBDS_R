...
#doesn't make senese to regress out ncounts here, because it is constructing a model based on ncounts. 
# variable.features.n = 2000  -is number of differentially expressed genes you're looking for. sometimes there are tools where you can express additional critiera for the genes you want to look at. 
```
...
#printing the name of the object is also helpful 

print(seurat_after_qc)
#it creates a new assay called SCT. the sct assay also now contains fewer genes - so there is some trimming going on!  

seurat_after_qc[[]]
#it has added an nCount_SCT column and nfeatureSCT column. the square brackets quickly enables us to see cell metadata 
```

Is there a change? 
Answer: yes - there's fewer genes
...
```{r}
VlnPlot(seurat_after_qc, features = c("nCount_RNA", "nCount_SCT"), ncol=2, same.y.lims = TRUE)
       
#VlnPlot(seurat_after_qc, features = c("nCount_RNA", "nCount_SCT"), ncol=2, same.y.lims = TRUE, group.by = "orig.ident")

VlnPlot(seurat_after_qc, features = c("nFeature_RNA", "nFeature_SCT"), ncol=2, same.y.lims = TRUE, group.by = "orig.ident")
#the addition of group.by orig.ident, allows us to look at the whole dataset. if you instead leave this parameter at default, the samples are stratified by cluster. 
...
```{r}
#activity aims to show there are some differences, though minor. these differential outcomes simply reflect changes in correction method (log norm vs sct)
```
...
Embeddings(seurat_after_qc, reduction = "sct.pca") %>% head()
```
...
Graphs(seurat_after_qc) #to check the names of the graphs so can specify below 
...
#table is tabulating - gives no. cells in each cluster using each method. E.g. at 0, 1, this is the no. cells in cluster 0 in one but cluster 1 in the other.  
...
p2 <- DimPlot(seurat_after_qc, reduction = 'sct.umap', pt.size = NULL, group.by = 'SCT_snn_res.0.5') + ggtitle("SCT UMAP")
p1 + p2
```
...
p1 <- FeaturePlot(seurat_after_qc, features = c("LYZ","FCGR3A","CD4","CD8A","GNLY","MS4A1","CST3","PPBP"), reduction = "sct.umap")
p2 <-  FeaturePlot(seurat_after_qc, features = c("LYZ","FCGR3A","CD4","CD8A","GNLY","MS4A1","CST3","PPBP"), reduction = "umap")
p1 + p2
```
...
Assays(seurat_after_qc)#check assay names

#seurat_markers_all <- FindAllMarkers() this would've been to replicate what was run this morning on the RNA assay

#you must remember to set the identify before the call. 
...
head(hto12.umis) #this is a count matrix. each row is a gene. each column is a droplet
dim(hto12.umis) #25339 genes and 30000 droplets 
...
head(hto12.htos) #columns are samples. it also has some metadata. rows are the barcode for the droplets. i.e. one barcode per cell (or, more accurately, per droplet)
dim(hto12.htos) #there's loads!

#NB, this is the matrix that has teh different antibody tag that were given to the different samples. So each sample name here replaces the name of the oligo used to identify that sample. 
#there will be lots of noise which is why the algorithm is needed. there is ambient RNA that may be similar to each of the barcodes. & other possible reasons why multiple barcodes are identified for each sell. 
```
...
```{r}
#there are many rows in the second file that we're not interested in because all values are zero. 

joint.bcs <- intersect(rownames(hto12.htos), colnames(hto12.umis)) #joins barcodes

length(joint.bcs) #can't use the dim function on a vector!
...
```{r}
hto12.htos.common <- t(hto12.htos[colnames(hto12_object),1:12]) #subset the hto dataframe; this is to remove metadata. we only want the HTO tags. The encircling t can transpose.
...
hto12_object <- HTODemux(hto12_object, 
                         assay = "HTO") #positive quantile is the p value cut off, deafult is 0.99

#it will print out the cutt offs that it has worked out for each label. it seems that the KG1 label was given out much more frequently. 
```
...
# HTO_maxID is the most probable identity. then it gives second most likely. then it gives a margin - when the difference between the two being assigned is small, then it is very likely to be a doublet. the final hash.ID is important because it tells you the final outcome, e.g. doublet, or label. 
```
...
RidgePlot(hto12_object, assay = "HTO", features = c("HEK-A", "K562-A", "THP1-A", "KG1")) #this is great as it shows really well how there is background and a clear real signal
```
...
FeatureScatter(hto12_object, feature1 = "HEK-A", feature2 = "THP1-A", group.by = "HTO_classification.global") #this includes the info on doublets, etc

FeatureScatter(hto12_object, feature1 = "HEK-A", feature2 = "THP1-A", group.by = "HTO_maxID") 

FeatureScatter(hto12_object, feature1 = "HEK-A", feature2 = "THP1-A", group.by = "HTO_secondID") #this should generally just be noise. 
