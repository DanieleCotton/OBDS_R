...
str(DNA_ALPHABET)
#this is just a list of things that are allowed in the 'DNA_ALPHABET' object
```
...
#type name of package in help then go to vignette link

#if you click on the package in packages tab, you can alos see the vignettes. there is a list of bullets, one of which says user guides, pacakge vignettes, and other documentation
```
...
```{r}
truseq_adapters <- readDNAStringSet("TruSeq3-PE-2.fa")
truseq_adapters

#NB fasta is just a text file. but by saying that it is a fasta file, when it is read, it is interpreted properly. E.g. it knows that the > represents a new header
```

``` {r}
str(truseq_adapters)

class(truseq_adapters) #this says what the class is (DNAStringSet) and also, helpfully tells you what package this object came from. 

typeof(truseq_adapters)
```
...
#it seems to be giving a list of genomes from different species. are they different sources / different builds? 
```
...
#with fixed = true, it will try to match the N to an N in the genome. but N represents ANY of the bases - so fixed =FALSE is required. The IUPAC ambiguity code then works. 
```
...
unique_transcripts <- unique(gtf_ACTB$transcript_id)
unique_transcripts

##or you can subsest again 

unique_transcripts2 <- subset(gtf_ACTB, gtf_ACTB$type == 'transcript')
unique_transcripts
...
```{r}
gtf_ACTB_exons <- subset(gtf_ACTB, gtf_ACTB$type == 'exon') 
gtf_ACTB_transcripts <- split(gtf_ACTB_exons, gtf_ACTB_exons$transcript_id) #take all the rows where type is exon. and for these rows, separate them based on the transcript id. thus, we get the number of exons for each transcript id. 
gtf_ACTB_transcripts

#after you do this, run
lengths(gtf_ACTB_transcripts)
#length of each element in the list of genomic ranges. 
...
#Entrez id often used for pathway analysis
...
> Answer:
> it works, but if you also loaded tidyverse, it might get confused about what select to use
...
#TIP, if you use one dataframe with ensembl, symbol, and entrezid, that would be really helpful as you can use a single dataframe, but keep switching to the different naming system column that you need for each analysis. 

#NB, map id takes a vector in and vector out. 
```
...
```{r}
assay_counts <- read.csv("/t1-data/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/counts.csv", row.names = "X") #or row.names = 1
...
```{r}
sample_metadata <- read.csv("/t1-data/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/sample_metadata.csv", row.names = "X")
head(sample_metadata)
```

```{r}
gene_metadata <- read.csv("/t1-data/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/gene_metadata.csv", row.names = "X")
head(gene_metadata)
```
...
```{r}
demo_se <- SummarizedExperiment(
  assays = list(counts = assay_counts), #this is a requirement 
