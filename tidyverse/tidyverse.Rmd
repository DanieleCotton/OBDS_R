...
iris_raw %>% 
  group_by(species) %>% 
  summarise(Sepal.Length.mean = mean(sepal_length), Sepal.Lenght.median = median(sepal_length), Sepal.Length.min = min(sepal_length), Sepal.Lenght.max = max(sepal_length))
...
iris_raw %>% 
    ...
    summarise(across(where(is.numeric), mean)) %>% 
    filter(sepal_length < 6) %>% 
    arrange(desc(sepal_length)) #not arrange(species, desc(sepal_length)), as this would sort by Sepal length
...
iris_raw %>% 
    ...
    select(species, sepal_area = area)
```
...
exprs_assay_long <- exprs_assay %>% 
      pivot_longer(cols = where(is.numeric), names_to = "sample")
...
sample_info <- sample_info %>%
    mutate(as_factor(grade, G1, G2, G3))
